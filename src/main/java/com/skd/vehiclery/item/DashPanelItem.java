...
public class DashPanelItem extends TooltipBlockItem {
    private static final Component TOOLTIP = Component.translatable("tooltip.block.vehiclery.dash_panel")
    ...
    public InteractionResult useOn(UseOnContext ctx) {
        ...
        if (state.is(VehicleryBlocks.SLOPE.require())) {
            if (!lvl.isClientSide()) {
                ctx.getLevel().setBlockAndUpdate(pos, VehicleryBlocks.SLOPE_WITH_DASH_PANEL.require()
        ...
        if (state.is(VehicleryBlocks.STEEP_SLOPE.require())) {
            if (!lvl.isClientSide()) {
                ctx.getLevel().setBlockAndUpdate(pos, VehicleryBlocks.STEEP_SLOPE_WITH_DASH_PANEL.require()
                        .withPropertiesOf(state));
            }
            afterPlace(ctx.getItemInHand(), lvl, ctx.getPlayer(), pos);
