...
public record HornSoundDefinition(Supplier<SoundEvent> sound, float loopStart, float loopEnd, List<Float> pitches) {
    ...
    public static HornSoundDefinition klaxon(int ... notes) {
        return new HornSoundDefinition(VehiclerySounds.KLAXON_HORN::require, 0.1916f, 0.9405f, pitchesOf(notes));
    }

    public static HornSoundDefinition disc(int ... notes) {
        return new HornSoundDefinition(VehiclerySounds.DISC_HORN::require, 0.09435f, 0.6605f, pitchesOf(notes));
    }

    public static HornSoundDefinition trumpet(int ... notes) {
        return new HornSoundDefinition(VehiclerySounds.TRUMPET_HORN::require, 0.151f, 0.95f, pitchesOf(notes));
    }

    public static HornSoundDefinition brass(int ... notes) {
        return new HornSoundDefinition(VehiclerySounds.BRASS_HORN::require, 0.2019f, 0.3138f, pitchesOf(notes));
    }

    public static HornSoundDefinition party(int ... notes) {
        return new HornSoundDefinition(VehiclerySounds.PARTY_HORN::require, -1, -1, pitchesOf(notes));
    }
